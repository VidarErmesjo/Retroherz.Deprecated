			...
			foreach (var arbiterId in ActiveEntities)
			{
				...
				if (arbiter.ray is not null)
				{
					...
					foreach (var candidateId in ActiveEntities.Where(id => id != arbiterId))
					{
						...
						var inflated = Predictive.BoundingRectangle(candidate.collider, candidate.transform, deltaTime);

						if (circle.Intersects(inflated))
						{
							var origin = Predictive.Vector(arbiter.collider, arbiter.transform, deltaTime);
							var target = Predictive.Vector(candidate.collider, candidate.transform, deltaTime);
